...
class Common extends AuthController
{
    ...
    public function homeStatics()
    {
        ...
        $todayUsers = (int)$userServices->todayAddVisits('today', 1);
        $yesterdayUsers = (int)$userServices->todayAddVisits('yesterday', 1);
        $totalUsers = (int)$userServices->count([]);
        $promoters = (int)$userServices->count(['is_promoter' => 1]);
        $pendingExtracts = (int)$extractServices->userExtractCount();
        $commissionBalance = (float)$userServices->sum([], 'brokerage_price');

        $info = [
            $this->statCard('新增用户', $todayUsers, $yesterdayUsers, '用户总数', $totalUsers),
            $this->statCard('分销员总数', $promoters, $promoters, '分销员', $promoters),
            $this->statCard('待审核提现', $pendingExtracts, $pendingExtracts, '待处理', $pendingExtracts),
            $this->statCard('用户佣金余额', $commissionBalance, $commissionBalance, '佣金余额', $commissionBalance),
        ];
        return app('json')->success(compact('info'));
    }

    protected function statCard(string $title, $today, $yesterday, string $totalName, $total): array
    {
        return [
            'title' => $title,
            'date' => '当前',
            'today' => $today,
            'yesterday' => $yesterday,
            'today_ratio' => $this->growth($today, $yesterday),
    ...
    */
    ...
    public function jnotice()
    {
        /** @var UserExtractServices $extractServices */
        $extractServices = app()->make(UserExtractServices::class);
        $reflectnum = $extractServices->userExtractCount();
        $value = [];
        if ($reflectnum != 0) {
            $value[] = [
