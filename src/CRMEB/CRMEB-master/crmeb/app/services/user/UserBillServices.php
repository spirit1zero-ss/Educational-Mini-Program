...
class UserBillServices extends BaseServices
{
    ...
    public function getCommissionList(array $where, int $limit = 0)
    {
        ...
        $fields = 'sum(IF(b.pm = 1 AND b.type <> \'extract_fail\', b.number, 0)) as income,'
            . 'sum(IF(b.pm = 0, b.number, 0)) as pay,'
            . 'u.nickname,u.phone,u.uid,u.now_money,u.brokerage_price,'
            . 'MAX(b.add_time) as time';
        [$count, $list] = $userUserBrokerage->getBrokerageList($where_data, $fields, $order_string, $limit);
