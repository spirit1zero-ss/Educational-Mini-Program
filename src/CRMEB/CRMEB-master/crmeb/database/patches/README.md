...
资料”和“确认银行卡提现到账”权限，并同步隐私政策文案；同时恢复“微信支付配置”
菜单、V3 支付及商家转账所需的表单字段。该云数据库兼容补丁只执行一次，不能在
同一数据库重复导入。部署代码前必须先导入该补丁，并在云托管以密钥方式配置
...
如果数据库已经执行过第 17 项，但后台“微信支付配置”只显示标签、表单区域空白，
再执行 `2026-07-31-payment-settings-form-repair.sql`。该修复只补齐或启用支付配置
分类和字段，不覆盖已经填写的商户号、密钥、证书或公钥，可安全重复执行。
