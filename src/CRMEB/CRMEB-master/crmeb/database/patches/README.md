...
微信商家转账需要公网 HTTPS 回调地址时，再执行
`2026-08-04-site-url-payment-settings.sql`。该补丁把现有 `site_url` 配置移动到
“微信支付配置”表单中，并保留当前值，可安全重复执行。云托管配置了
`PHP_SITE_URL` 时后端优先使用环境变量；未配置时才读取后台保存的地址。

## 2026-08-04 小程序协议同意记录

部署本次小程序登录协议留痕代码前，执行：

18. `2026-08-04-miniapp-user-consent.sql`

该补丁新增 `eb_miniapp_user_consent` 表，按用户、协议组合和协议版本幂等记录同意
时间。接口只接受已登录小程序用户调用，不采集手机号；服务端时间作为正式留痕时间。
以后协议内容发生实质变化时，必须同步提升小程序中的协议版本号。

## 2026-08-04 对外品牌名称

已有数据库如仍显示 CRMEB 默认站点名或初始账号名称，可执行：

19. `2026-08-04-public-branding.sql`

该补丁不包含 `USE` 语句，适合权限受限的云数据库；可安全重复执行。它只修改仍为
CRMEB 原始种子值的站点名、默认客服、初始管理员和 UID 1 用户，不覆盖已经人工
设置的名称。Logo 文件沿用原配置路径，由本次代码部署直接替换，无需写入图片 URL。
