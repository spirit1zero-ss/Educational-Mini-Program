<template>
	<!-- #ifdef APP-PLUS || H5 -->
	<text v-if="!mvpEnabled" class="acea-row row-center-wrapper cartf iconfont icon-kefu3" :style="{ top: top + 'px'}" @click="goCustomer"
		@touchmove.stop.prevent="setTouchMove"></text>
	<!-- #endif -->
	<!-- #ifdef MP -->
	<view v-if="!mvpEnabled && routineContact == 0" class="acea-row row-center-wrapper cartf iconfont icon-kefu3"
	...
	<button class="acea-row row-center-wrapper cartf iconfont icon-kefu3" open-type='contact'
		...
		v-else-if="!mvpEnabled && routineContact==1 && !goodsCon"></button>
	<button class="acea-row row-center-wrapper cartf iconfont icon-kefu3" open-type='contact'
