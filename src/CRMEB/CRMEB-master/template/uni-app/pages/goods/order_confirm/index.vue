		...
		methods: {
			...
			getConfirm: function() {
				...
				orderConfirm(data).then(res => {
					...
					that.$set(that, 'priceGroup', priceGroup);
					that.$set(that, 'totalPrice', that.$util.$h.Add(parseFloat(priceGroup.totalPrice),
						parseFloat(priceGroup.storePostage)));
			...
			getBargainId: function() {
				...
				if (that.isCoreScope) {
					...
					that.$set(that, 'advanceId', 0);
					that.$set(that, 'seckillId', 0);
					return;
				}
