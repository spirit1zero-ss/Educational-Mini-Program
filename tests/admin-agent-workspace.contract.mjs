...
const registrationServices = read('src/CRMEB/CRMEB-master/crmeb/app/services/miniapp/TrainingCampRegistrationServices.php')
...
assert.match(userList, /command="14"/)
assert.match(userList, /撤销付费会员/)
assert.match(userList, /设为付费会员/)
assert.match(userList, /distribution_only: 'membership'/)
assert.match(userController, /\$distributionOnly === 'membership'/)
assert.match(userServices, /function updatePaidMembership/)
assert.match(userServices, /'is_money_level' => 3/)
assert.match(registrationServices, /'canRegister' => !empty\(\$memberStatus\['isMember'\]\)/)
