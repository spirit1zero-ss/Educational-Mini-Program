...
assert.match(miniService, /maskGeneratedWechatNickname/)
assert.match(miniService, /preg_replace_callback/)
assert.match(miniService, /\(\?<\!\[A-Za-z0-9_\]\)\(\[wv\]x\)\(\\d\{6\}\)\(\?!\\d\)/)
assert.match(miniService, /'desc'\s*=>\s*\$mark/)
assert.match(miniService, /'mark'\s*=>\s*\$mark/)
...
assert.match(distributionCenterPage, /member\.nickname/)
assert.match(distributionCenterPage, /member\.avatar/)
assert.match(distributionCenterView, /待抵扣佣金/)
assert.match(distributionCenterView, /退款累计扣回/)
assert.match(distributionCenterView, /profile\.nickname/)
assert.match(distributionCenterView, /profile\.avatar/)
assert.match(miniService, /'nickname'\s*=>\s*\(string\)\(\$user\['nickname'\]/)
assert.match(miniService, /'avatar'\s*=>\s*\(string\)\(\$user\['avatar'\]/)
...
assert.match(minePage, /key:\s*'registration'[\s\S]*?memberOnly:\s*true/)
assert.match(mineView, /wx:if="\{\{!item\.hidden && \(!item\.memberOnly \|\| isMember\)\}\}"/)
...
assert.match(agentManage, /useDefaultAvatar/)
